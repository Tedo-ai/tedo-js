...
import type {
  ...
  CreateChargeParams,
  ListChargesParams,
  ChargeCheckoutResult,
  CreateChargeCheckoutParams,
  Invoice,
  CreateInvoiceParams,
  ListInvoicesParams,
...
export class BillingService {
  ...
  async listCharges(
    ...
    return this.req("GET", "/billing/v1/charges", undefined, query);
  }

  async createCharge(params: CreateChargeParams): Promise<Charge> {
    return this.req<Charge>("POST", "/billing/v1/charges", params);
  }

  async getCharge(id: string): Promise<Charge> {
    return this.req<Charge>("GET", `/billing/v1/charges/${id}`);
  }

  async createChargeCheckout(
    chargeId: string,
    params?: CreateChargeCheckoutParams,
  ): Promise<ChargeCheckoutResult> {
    return this.req<ChargeCheckoutResult>(
      "POST",
