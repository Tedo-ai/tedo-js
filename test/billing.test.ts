...
describe("BillingService", () => {
  ...
  describe("charges", () => {
    it("listCharges sends GET /billing/v1/charges with query", async () => {
      const { client, transport } = createTestClient();
      transport.enqueue(200, {
        charges: [{ id: "ch_1", customer_id: "cus_1" }],
        total: 1,
      });

      const list = await client.billing.listCharges({
        customer_id: "cus_1",
        limit: 50,
      });
      ...
      expect(transport.lastRequest.method).toBe("GET");
      expect(transport.lastRequest.path).toBe("/billing/v1/charges");
      expect(transport.lastRequest.query).toEqual({
        customer_id: "cus_1",
        limit: "50",
      });
    ...
    it("createCharge sends POST /billing/v1/charges", async () => {
      const { client, transport } = createTestClient();
      transport.enqueue(201, { id: "ch_1", total: 1499 });

      await client.billing.createCharge({
        customer_id: "cus_1",
        currency: "EUR",
        lines: [
          {
    ...
    it("createInvoice aliases to the charge endpoint", async () => {
      const { client, transport } = createTestClient();
      transport.enqueue(201, { id: "ch_1", total: 1499 });
    ...
    it("createChargeCheckout sends POST /billing/v1/charges/:id/checkout", async () => {
      const { client, transport } = createTestClient();
      transport.enqueue(201, {
        payment_id: "pay_1",
        charge_id: "ch_1",
        checkout_url: "https://billing.tedo.ai/checkout",
      });

      await client.billing.createChargeCheckout("ch_1", {
        redirect_url: "https://example.com/return",
      });

      expect(transport.lastRequest.method).toBe("POST");
      expect(transport.lastRequest.path).toBe(
        "/billing/v1/charges/ch_1/checkout",
      );
    });
  });
});
